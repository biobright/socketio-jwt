  ...
  return function (socket) {
    ...
    if(options.required){
      var auth_timeout = setTimeout(function () {
          var error = new UnauthorizedError('authenticate timeout', {
              message: 'Timeout for authentication has passed!'
          });
        socket.emit('unauthorized', error);
        socket.disconnect('unauthorized');
        if (server.$emit) {
            server.$emit('unauthorized', socket, error);
        } else {
            ...
            var namespace = (server.nsps && socket.nsp &&
                ...
                server.sockets;
            // explicit namespace
            namespace.emit('unauthorized', socket, error);
        }
    ...
    socket.on('authenticate', function (data) {
      ...
      getSecret(socket.request, options.secret, data.token, onSecretReady, options.complete);
    });
  };
}
...
function authorize(options, onConnection) {
  ...
  return function(data, accept){
    ...
    getSecret(req, options.secret, token, onSecretReady, options.complete);
  };
}

function getSecret(request, secret, token, callback, complete) {
  if (typeof secret === 'function') {
    ...
    var decodedToken = jwt.decode(token, {complete: complete || false});
